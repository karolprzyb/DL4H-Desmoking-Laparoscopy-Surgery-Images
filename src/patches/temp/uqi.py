...
def _uqi_compute(
    ...
    # print("input list")
    # print(torch.isnan(input_list).any())
    outputs = F.conv2d(input_list, kernel, groups=channel)
    output_list = outputs.split(preds.shape[0])
    # print(len(output_list))
    # print(preds.shape[0])
    # print("output list")
    # print(torch.isnan(output_list).any())
    ...
    # DIAGNOSTIC CODE TO SEE INSTABILITY
    # if (torch.abs(uqi_idx[0]) == torch.tensor((torch.inf))).any():
    #     getinfs = (torch.abs(uqi_idx[0]) == torch.tensor((torch.inf))).nonzero().tolist()
    #     print("start")
    #     print(len(getinfs))
    #     leninfs = min(len(getinfs), 5)
    #     getinfs = tuple(getinfs[:leninfs])
    #     print(getinfs)
    #     for mydx in range(0, leninfs):
    #         for myx in range(0,5):
    #             print(output_list[myx].shape)
    #             print(output_list[myx][0].cpu()[tuple(getinfs[mydx])])
    #         print(mu_pred_target[0].cpu()[tuple(getinfs[mydx])])
    #         print(upper[0].cpu()[tuple(getinfs[mydx])])
    #         print(lower[0].cpu()[tuple(getinfs[mydx])])
    #         print(mu_pred_sq[0].cpu()[tuple(getinfs[mydx])])
    #         print(mu_target_sq[0].cpu()[tuple(getinfs[mydx])])

    # nanidx = -1
    # for chkidx in range(0, len(uqi_idx)):
    #     if (uqi_idx[chkidx].isnan()).any():
    #         nanidx = chkidx
    #         break
    # if nanidx != -1:
    #     if (uqi_idx[nanidx].isnan()).any():
    #         getinfs = uqi_idx[nanidx].isnan().nonzero().tolist()
    #         print("start - NAN")
    #         print(len(getinfs))
    #         leninfs = min(len(getinfs), 5)
    #         getinfs = tuple(getinfs[:leninfs])
    ...
    #             for myx in range(0,5):
    #                 print(output_list[myx].shape)
    #                 print(output_list[myx][nanidx].cpu()[tuple(getinfs[mydx])])
    #             print(mu_pred_target[nanidx].cpu()[tuple(getinfs[mydx])])
    #             print(upper[nanidx].cpu()[tuple(getinfs[mydx])])
    #             print(lower[nanidx].cpu()[tuple(getinfs[mydx])])
    #             print(mu_pred_sq[nanidx].cpu()[tuple(getinfs[mydx])])
    #             print(mu_target_sq[nanidx].cpu()[tuple(getinfs[mydx])])
    #         print("=================End=================")

    # test1 = lower-1e-9 == 0
    # test2 = (mu_pred_sq+mu_target_sq)==0
    # if test1.any():
    #     print("test1 error is zero")
    # if test2.any():
    #     print("test2 is zero ERROR")
    uqi_idx = uqi_idx[..., pad_h:-pad_h, pad_w:-pad_w]
    # if (uqi_idx < 0).any():
    #     print("What... less than zero?")
    # if (uqi_idx > 1).any():
    #     print("More than one. So clamp should fix?")
    uqi_idx = torch.clamp(uqi_idx, min=-1.0, max=1.0)
    # if (uqi_idx > 1).any():
    #     print("Clamp did not fix?")
